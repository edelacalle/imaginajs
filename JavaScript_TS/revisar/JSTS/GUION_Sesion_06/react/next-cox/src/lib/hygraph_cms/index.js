...
var oData= {"en":{}};
  ...
  const getSlugsbyAxiosAPICall = async () => {
    try {
      ...
      const res = await fetch ( HYGRAPH_URL, opts );
      const jRes = await res.json();

      var oRet = []
      jRes.data.posts.map((e)=>oRet.push(e.slug)); 
      return oRet;    
    }
    catch (err) {
      console.log('ERROR HYGRAPH REQUEST', err);
    }
    finally {
...
const getPostAxiosAPICall = async (id) => {
    try {
      ...
      var oRet = {
        ...
        "en":{
            id: id,
            title:jRes.data.post.title,
            subtitle:jRes.data.post.subtitle,
            content:jRes.data.post.content.html
        },
        "es":{
            id: id,
            title:jRes.data.post.localizations[0].title,
            subtitle:jRes.data.post.localizations[0].subtitle,
            content: jRes.data.post.localizations[0].content.html
        }
      }
      return oRet;    
      ...
      console.log('ERROR DURING HYGRAPH  REQUEST', err);
    }
    finally {
